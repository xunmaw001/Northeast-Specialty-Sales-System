        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '农户编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.nonghuUuidNumber" placeholder="农户编号" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '农户姓名' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.nonghuName" placeholder="农户姓名" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('nonghu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/dbtutechan/upload/nonghuMuBan.xls"
                        >批量导入农户数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('nonghu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="nonghuUploadSuccess"
                                :on-error="nonghuUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('nonghu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "nonghu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nonghuUuidNumber"
                                   header-align="center"
                                   label="农户编号">
                        <template slot-scope="scope">
                            {{scope.row.nonghuUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="农户身份证号">
                        <template slot-scope="scope">
                            {{scope.row.nonghuIdNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="nonghuPhoto"
                                     header-align="center"
                                     width="200"
                                     label="农户照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.nonghuPhoto">
                                <img :src="$base.url+scope.row.nonghuPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('nonghu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('nonghu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('nonghu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('nonghu','修改')" type="success" size="mini" @click="jinqiyong(scope.row.id,scope.row.jinyongTypes)">{{scope.row.jinyongTypes == 1?'禁用账户':'启用账户'}}</el-button>

                            <el-button v-if="isAuth('nonghu','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                ...
                //本表字段
                     '账户': "username",
                     '农户编号': "nonghuUuidNumber",
                     '农户姓名': "nonghuName",
                     '农户手机号': "nonghuPhone",
                     '农户身份证号': "nonghuIdNumber",
                     '农户照片': "nonghuPhoto",
                     '性别': "sexValue",
                     '农户邮箱': "nonghuEmail",
                     '余额': "newMoney",
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'nonghu',//当前表表名,
                        sumColum : 'nonghu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'nonghu_name',//分组字符串字段
                        // types : 'nonghu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.nonghuUuidNumber!= '' && this.searchForm.nonghuUuidNumber!= undefined) {
                    params['nonghuUuidNumber'] = '%' + this.searchForm.nonghuUuidNumber + '%'
                }
         
                if (this.searchForm.nonghuName!= '' && this.searchForm.nonghuName!= undefined) {
                    params['nonghuName'] = '%' + this.searchForm.nonghuName + '%'
                }
                ...
                params['nonghuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "nonghu/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            nonghuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "nonghu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            jinqiyong(id,jinyongTypes) {
                ...
                _this.$confirm(msg, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"}).then(() => {
                    _this.$http({url: "nonghu/update",method: "post",data: {id:id,jinyongTypes:jinyongTypes,}}).then(({data}) => {
                        if(data && data.code === 0){
