    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(shangpin !=null){
            ...
            //级联表 农户
            //级联表
            NonghuEntity nonghu = nonghuService.selectById(shangpin.getNonghuId());
            if(nonghu != null){
            BeanUtils.copyProperties( nonghu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "nonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
    ...
    */
    ...
    public R save(@RequestBody ShangpinEntity shangpin, HttpServletRequest request){
        ...
        if(false)
            ...
            shangpin.setNonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<ShangpinEntity> queryWrapper = new EntityWrapper<ShangpinEntity>()
            .eq("nonghu_id", shangpin.getNonghuId())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            ShangpinEntity shangpinEntity = new ShangpinEntity();
//                            shangpinEntity.setNonghuId(Integer.valueOf(data.get(0)));   //农户 要改的
...
//                            shangpinEntity.setShangpinAddress(data.get(0));                    //原产地 要改的
...
//                            shangpinEntity.setBangfuTypes(Integer.valueOf(data.get(0)));   //帮扶产品 要改的
        ...
        ShangpinEntity shangpin = shangpinService.selectById(id);
            if(shangpin !=null){
                ...
                //级联表
                    NonghuEntity nonghu = nonghuService.selectById(shangpin.getNonghuId());
                if(nonghu != null){
                    BeanUtils.copyProperties( nonghu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "nonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setNonghuId(nonghu.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
    ...
    */
    ...
    public R add(@RequestBody ShangpinEntity shangpin, HttpServletRequest request){
        ...
        Wrapper<ShangpinEntity> queryWrapper = new EntityWrapper<ShangpinEntity>()
            ...
            .eq("shangpin_types", shangpin.getShangpinTypes())
            .eq("shangpin_erji_types", shangpin.getShangpinErjiTypes())
            .eq("bangfu_types", shangpin.getBangfuTypes())
            .eq("shangpin_kucun_number", shangpin.getShangpinKucunNumber())
            .eq("shangpin_price", shangpin.getShangpinPrice())
            .eq("shangpin_cangku", shangpin.getShangpinCangku())
