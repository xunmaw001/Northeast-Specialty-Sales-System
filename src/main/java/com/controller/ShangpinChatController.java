...
, "nonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setYonghuId(yonghu.getId());
            }
            //级联表 农户
            //级联表
            NonghuEntity nonghu = nonghuService.selectById(shangpinChat.getNonghuId());
            if(nonghu != null){
            BeanUtils.copyProperties( nonghu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "nonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setNonghuId(nonghu.getId());
            }
            //修改对应字典表字段
    ...
    */
    ...
    public R save(@RequestBody ShangpinChatEntity shangpinChat, HttpServletRequest request){
        ...
        if(false)
            ...
            shangpinChat.setNonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<ShangpinChatEntity> queryWrapper = new EntityWrapper<ShangpinChatEntity>()
            .eq("yonghu_id", shangpinChat.getYonghuId())
            .eq("nonghu_id", shangpinChat.getNonghuId())
...
//        else if("农户".equals(role))
//            shangpinChat.setNonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(shangpinChat.getShangpinChatIssueText()) || "null".equals(shangpinChat.getShangpinChatIssueText())){
                shangpinChat.setShangpinChatIssueText(null);
        }
        if("".equals(shangpinChat.getShangpinChatReplyText()) || "null".equals(shangpinChat.getShangpinChatReplyText())){
                shangpinChat.setShangpinChatReplyText(null);
        }
...
//                            shangpinChatEntity.setNonghuId(Integer.valueOf(data.get(0)));   //回答人 要改的
    ...
    public R detail(@PathVariable("id") Integer id, HttpServletRequest request){
        ...
        ShangpinChatEntity shangpinChat = shangpinChatService.selectById(id);
            if(shangpinChat !=null){
                ...
                if(yonghu != null){
                    BeanUtils.copyProperties( yonghu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney", "yonghuId"
, "nonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setYonghuId(yonghu.getId());
                }
                //级联表
                    NonghuEntity nonghu = nonghuService.selectById(shangpinChat.getNonghuId());
